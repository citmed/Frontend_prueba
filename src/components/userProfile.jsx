...
export default function Profile() {
  ...
  useEffect(() => {
    const fetchInfo = async () => {
      try {
        const res = await axios.get("https://backend-prueba-1-pj2l.onrender.com/api/info-user", {
  ...
  const handleSave = async () => {
    try {
      const res = await axios.put("https://backend-prueba-1-pj2l.onrender.com/api/info-user", formData, {
        headers: { Authorization: `Bearer ${token}` },
      });
      setFormData(res.data);
      setIsEditing(false); 
